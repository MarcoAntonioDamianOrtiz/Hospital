      ...
      <div class="map-container">
        ...
        <div v-if="!mapboxReady" class="map-fallback">
          <div class="fallback-inner">
            ...
            <p class="fallback-sub" style="margin-top:0.5rem">Mientras tanto, selecciona un hospital de la lista.</p>
          </div>
        </div>
      ...
      <div class="hospital-list">
        ...
        <div class="list-items">
          <button
            ...
            :key="hospital.id"
            class="hospital-item"
            :class="{ selected: selectedId === hospital.id }"
            @click="selectHospital(hospital)"
          >
            <div class="hosp-top">
              <span class="type-badge" :style="{ background: typeColors[hospital.type] + '22', color: typeColors[hospital.type], borderColor: typeColors[hospital.type] + '55' }">
                {{ hospital.type }}
              </span>
            ...
            <div class="hosp-stats">
              ...
              <div class="hosp-occ-bar">
                <div class="occ-fill" :style="{ width: occupancyPct(hospital) + '%', background: occupancyColor(hospital) }"></div>
              </div>
            </div>
          </button>
        </div>
...
const filteredHospitals = computed(() => {
  ...
  return HOSPITALS.filter(h =>
    h.name.toLowerCase().includes(q) || h.city.toLowerCase().includes(q) || h.type.toLowerCase().includes(q)
  )
})

function occupancyPct(h: Hospital) { return hospitalOccupancy(h) }
function occupancyColor(h: Hospital) {
...
function selectHospital(h: Hospital) {
  ...
  // Centrar mapa en el hospital seleccionado si está activo
  if (mapInstance) {
...
async function initMapbox() {
  ...
  try {
    // Importación dinámica para no romper el build si no está instalado
...
.panel-title { font-size: 0.9rem; font-weight: 600; color: var(--text-secondary); margin: 0; }
.map-sub { font-size: 0.75rem; color: var(--text-muted); margin-top: 0.2rem; }

.btn-close {
  padding: 0.35rem; background: transparent; border: 1px solid var(--border);
...
.fallback-title { font-size: 0.95rem; font-weight: 600; color: var(--text-secondary); }
.fallback-sub { font-size: 0.78rem; color: var(--text-muted); line-height: 1.5; }
.code-block {
  display: block; background: var(--bg-card); border: 1px solid var(--border);
  padding: 0.4rem 0.75rem; border-radius: var(--radius-sm);
  font-size: 0.75rem; color: var(--text-accent); font-family: var(--font-mono);
}
...
.list-items { flex: 1; overflow-y: auto; padding: 0.5rem; }

.hospital-item {
  width: 100%; text-align: left; padding: 0.75rem;
  background: transparent; border: 1px solid transparent;
  border-radius: var(--radius-sm); cursor: pointer;
  transition: all 0.15s; margin-bottom: 0.3rem;
}
.hospital-item:hover { background: var(--bg-card); border-color: var(--border); }
.hospital-item.selected { background: var(--accent-glow); border-color: rgba(59,130,246,0.4); }

.hosp-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.3rem; }
.type-badge {
  font-size: 0.62rem; font-weight: 700; padding: 0.15rem 0.45rem;
  border-radius: 100px; border: 1px solid; letter-spacing: 0.05em;
}
.hosp-city { font-size: 0.7rem; color: var(--text-muted); }
.hosp-name { font-size: 0.8rem; font-weight: 600; color: var(--text-primary); line-height: 1.3; margin-bottom: 0.5rem; }

.hosp-stats { display: flex; gap: 0.75rem; align-items: center; flex-wrap: wrap; }
.hosp-stat { display: flex; flex-direction: column; gap: 0.1rem; }
